...
const designation = () => {
  //meta title
  document.title = "Knowledge pro | Designation"
  ...
  const getDesignationDetails = () => {
    ...
    axios.get("http://localhost:8080/admin/adminPage/getAllDesignation", {
      headers: {
  ...
  /////------post---------------------------//////////////////////////////////
  const postDesignationDetails = () => {
    axios.post("http://localhost:8080/admin/adminPage/addDesignation", formdetails, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const deleteDesignationDetails = () => {

    axios.delete(`http://localhost:8080/admin/adminPage/deleteDesignation/${deleteId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const getEditEntry = (user) => {
    console.log(localStorage.getItem("token"))
    axios.get(`http://localhost:8080/admin/adminPage/getDesignationById/${user.id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const updateDesignationDetails = () => {
    let req = {
      ...
      order: formdetails.order
    }

    axios.put("http://localhost:8080/admin/adminPage/updateDesignation", req, {
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Admin" breadcrumbItem="Designation Entry" />
                  ...
                  <div className="table-rep-plugin">
                    ...
                    >
                      <Table
                        ...
                        <thead>
                          <tr>
                            <td align="right"><span className="mandatory">*</span> Name:</td>
                            <td align="left"><input type="text" name="name" value={formdetails.name} onChange={(e) => { handleInputChange(e) }}></input></td>


                            <td align="right"><span className="mandatory">*</span>Orders:</td>
                            <td align="left"><input type="text" name="order" value={formdetails.order} onChange={(e) => { handleInputChange(e) }}></input></td>
                          </tr>
                        </thead>
                      </Table>
                      <div className="hstack gap-3 align-center">
                        <button type="button" className="btn btn-primary" onClick={isEdit ? updateDesignationDetails : postDesignationDetails}>{isEdit ? "Update" : "Submit"}</button>
        ...
        <Modal show={show} centered={true} onHide={handleClose} animation={false}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={(e) => { deleteDesignationDetails() }}>
              Delete
            </Button>
          </Modal.Footer>
        </Modal>
