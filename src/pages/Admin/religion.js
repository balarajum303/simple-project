...
const religion = () => {
  //meta title
  document.title = "Knowledge pro | Religion"
  ...
  const createReligion = () => {
    axios.post("http://localhost:8080/admin/adminPage/addReligion",formdetails, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const deleteReligion = () => {
    ...
    axios.delete(`http://localhost:8080/admin/adminPage/deleteReligion/${deleteId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const getEditEntry = (user) => {
    console.log(localStorage.getItem("token"))
    axios.get(`http://localhost:8080/admin/adminPage/getReligionById/${user.id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const updateReligion = () => {
    ...
    axios.put("http://localhost:8080/admin/adminPage/updateReligion",req, {
  ...
  const handlesearch = (event) => {
    ...
    if (getsearch.length > 0) {
      const searchdata = religion.filter((item) => item.name.toLowerCase().includes(getsearch));
      setReligion(searchdata)
    } else {
      setReligion(filterdata);
    }
  }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Admin" breadcrumbItem="Religion Entry" />
                  ...
                  <div className="table-rep-plugin">
                    ...
                    >
                      ...
                      <div className="hstack gap-3 align-center">
                        <button type="button" className="btn btn-primary" onClick={isEdit ? updateReligion : createReligion}>{isEdit ? "Update" : "Submit"}</button>
                      ...
                      >
                        ...
                        <Tbody>
                          {religion.map((user, index) => <Tr key={index}>
                            <Td >{index + 1}</Td>
                            <Td>{user.name}</Td>
        ...
        <Modal show={show} centered={true} onHide={handleClose} animation={false}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={(e) => { deleteReligion() }}>
              Delete
            </Button>
          </Modal.Footer>
        </Modal>
