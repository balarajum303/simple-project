...
const cost_category = () => {
  //meta title
  document.title = "Knowledge pro | Cost Category"
  ...
  /////------post---------------------------//////////////////////////////////
  const createCountry = () => {
    axios.post("http://localhost:8080/admin/adminPage/addCostCategory", formdetails, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const deleteCountry = () => {
    ...
    axios.delete(`http://localhost:8080/admin/adminPage/deleteCostCategory/${deleteId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const getEditEntry = (user) => {
    console.log(localStorage.getItem("token"))
    axios.get(`http://localhost:8080/admin/adminPage/getCostCategoryById/${user.id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const updateCountry = () => {
    ...
    axios.put("http://localhost:8080/admin/adminPage/updateCostCategory", req, {
  ...
  const handlesearch = (event) => {
    ...
    if (getsearch.length > 0) {
      const searchdata = countryDetails.filter((item) => item.name.toLowerCase().includes(getsearch));
      setcountryDetails(searchdata)
    } else {
      setcountryDetails(filterdata);
    }
  }
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Admin" breadcrumbItem="Cost Category Entry" />
                  ...
                  <div className="table-rep-plugin">
                    ...
                    >
                      <Table
                        ...
                        <thead>
                          <tr>
                            <td align="right"><span className="mandatory">*</span>Cost Category Name:</td>
                      ...
                      <div className="hstack gap-3 align-center">
                        <button type="button" className="btn btn-primary" onClick={isEdit ? updateCountry : createCountry}>{isEdit ? "Update" : "Submit"}</button>
                      ...
                      >
                        <Thead>
                          <Tr>
                            ...
                            <Th data-priority="3">Cost Category Name</Th>
                            <Th data-priority="1">Edit</Th>
                            <Th data-priority="3">Delete</Th>
                          </Tr>
                        </Thead>
                        <Tbody>
                          {countryDetails.map((user, index) => <Tr key={index}>
                            <Td >{index + 1}</Td>
                            <Td>{user.name}</Td>
        ...
        <Modal show={show} centered={true} onHide={handleClose} animation={false}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={(e) => { deleteCountry() }}>
              Delete
            </Button>
          </Modal.Footer>
        </Modal>
