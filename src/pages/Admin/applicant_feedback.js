...
const applicant_feedback = () => {
  //meta title
  document.title = "Knowledge pro | Applicant Feedback"
  ...
  const getApplicantFeedback = () => {
    console.log(localStorage.getItem("token"))
    axios.get("http://localhost:8080/admin/adminPage/getAllApplicantFeedback", {
      headers: {
  ...
  /////------post---------------------------//////////////////////////////////
  const postApplicantFeedback = () => {
    axios.post("http://localhost:8080/admin/adminPage/addApplicantFeedback", formdetails, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const deleteApplicantFeedback = () => {

    axios.delete(`http://localhost:8080/admin/adminPage/deleteApplicantFeedback/${deleteId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const getEditEntry = (user) => {
    console.log(localStorage.getItem("token"))
    axios.get(`http://localhost:8080/admin/adminPage/getApplicantFeedbackById/${user.id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  ////////----- update----////////////
  const updateApplicantFeedback = () => {
    let req = {
      id: formdetails.id,
      name: formdetails.name
    }

    axios.put("http://localhost:8080/admin/adminPage/updateApplicantFeedback", req, {
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Admin" breadcrumbItem="Applicant Feedback" />
                  ...
                  <div className="table-rep-plugin">
                    ...
                    >
                      <Table
                        ...
                        <thead>
                          <tr>
                            <td align="right"><span className="mandatory">*</span>Applicant Feedback:</td>
                      ...
                      <div className="hstack gap-3 align-center">
                        <button type="button" className="btn btn-primary" onClick={isEdit ? updateApplicantFeedback : postApplicantFeedback}>{isEdit ? "Update" : "Submit"}</button>
                      ...
                      >
                        <Thead>
                          <Tr>
                            ...
                            <Th data-priority="3">Applicant Feedback</Th>
        ...
        <Modal show={show} centered={true} onHide={handleClose} animation={false}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={(e) => { deleteApplicantFeedback() }}>
              Delete
            </Button>
          </Modal.Footer>
        </Modal>
...
export default applicant_feedback
