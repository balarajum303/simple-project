...
const certificate_request_purpose = () => {
  //meta title
...
///////// delete/////////////
 ...
 const handleShow = () => setShow(true);
  ...
  const getReguest=()=>{
   ...
   axios.get("http://localhost:8080/admin/adminPage/getAllCertificateReq", {
    headers: {
  ...
  /////------post---------------------------//////////////////////////////////
  const postReguest=()=>{
    axios.post("http://localhost:8080/admin/adminPage/addCertificateReq",formdetails,{
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const deleteReguest=(user)=>{
    console.log("balll",user.id)
    axios.delete(`http://localhost:8080/admin/adminPage/deleteCertificateReq/${user.id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`,
        'Content-Type': 'application/json',
  ...
  const clearForm=()=>{
   ...
   const restDataHandler=()=>{
    ...
    const getEditEntry=(user)=>{
      console.log(localStorage.getItem("token"))
    axios.get(`http://localhost:8080/admin/adminPage/getCertificateReqById/${user.id}`, {
     headers: {
       'Authorization': `Bearer ${localStorage.getItem("token")}`,
       'Content-Type': 'application/json',
  ...
  const updateReguest=()=>{
    ...
    axios.put("http://localhost:8080/admin/adminPage/updateCertificateReq",req,{
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Admin" breadcrumbItem=" Certificate Request Purpose " />
                    ...
                    >
                      <Table
                        className="table table-striped table-bordered headerTable">
                           <thead>
                        <tr>
                          <td align="right"><span className="mandatory">*</span>Purpose Name:</td>
                       ...
                       <div className="hstack gap-3 align-center">
                            <button type="button" className="btn btn-primary" onClick={isEdit ? updateReguest: postReguest}>{isEdit ? "Update" : "Submit"}</button>
                      ...
                      >
                        <Thead>
                          <Tr>
                            ...
                            <Th data-priority="3">Purpose Name</Th>
                        ...
                        <Tbody>
                          {details.map((user,index)=> <Tr key={index}>
                            ...
                            <Td><button style={{border:"none"}}><img src={deleteIcon} onClick={(e)=>{deleteReguest(user,user.id)}}/></button></Td>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary"  onClick={(e)=>{deleteReguest(user,user.id)}}>
            delete Changes
          </Button>
        </Modal.Footer>
      </Modal> */}
...
export default certificate_request_purpose
